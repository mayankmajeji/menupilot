...
class Menu_Importer {
	...
	private function create_menu_item( int $menu_id, array $item, int $parent_id = 0 ) {
		...
		if ( isset( $item['custom_mapping'] ) && is_array( $item['custom_mapping'] ) ) {
			...
			if ( 'custom' === $custom_type || 0 === $custom_id ) {
				// User wants to keep as custom link.
				$type      = 'custom';
				$object    = 'custom';
				$object_id = 0;
			} elseif ( 'post' === $custom_type || 'page' === $custom_type ) {
				// User mapped to a specific post/page.
				$type      = 'post_type';
				$object    = $custom_type;
				$object_id = $custom_id;
			} elseif ( 'category' === $custom_type ) {
	...
	private function normalize_url( string $url ): string {
		...
		if ( 0 === strpos( $url, $this->source_url ) ) {
			...
			if ( 0 === strpos( $url, $source_no_slash ) ) {
