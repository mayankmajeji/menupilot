...
jQuery( document ).ready(
	function ($) {
		...
		$( document ).on(
			"click",
			".mp-modal-close, #mp-modal-cancel",
			function () {
				closeImportModal();
			}
		);
			...
			function (e) {
				...
				try {
					...
					if (menuData.menu && menuData.menu.items) {
						...
						menuData.menu.items = originalItems.filter(
							function (item, index) {
						...
						menuData.menu.items.forEach(
							function (item, _newIndex) {
									// Find original index
						...
						const removedIds = removedItems.map(
							function (idx) {
			...
			btn.addEventListener(
				...
				function () {
					...
					if (navigator.clipboard && navigator.clipboard.writeText) {
						navigator.clipboard.writeText( info ).then( showCopied ).catch(
							function () {
								fallbackCopy( info );
							}
						);
